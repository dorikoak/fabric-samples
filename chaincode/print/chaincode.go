...
func (t *SimpleAsset) updatePrint(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	printerVal.Black = 10
	printerVal.Cyan = printerVal.Cyan - 1
	printerVal.Magenta = printerVal.Magenta - 1
	printerVal.Yellow = printerVal.Yellow - 1
	printerVal.Drum = printerVal.Drum - 1
	printerVal.ErrorCode = 0
	printerVal.Paper = printerVal.Paper + 3
